    ...
    <div className="flex gap-1 mr-2 bg-base-100/50 p-1.5 rounded-xl border border-base-content/5">
      {colors.map((c) => (
        <button
          ...
          className={`w-4 h-4 rounded-full transition-all duration-300 ${primaryColor === c.value ? 'scale-125 ring-2 ring-base-content/50' : 'hover:scale-110'}`}
...
const ThemeToggleWithIcon = () => {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-3 py-2 rounded-xl border border-base-content/10 bg-base-100/50 hover:bg-base-200/50 text-base-content/70 hover:text-primary transition-all shadow-sm"
      title="Toggle Theme"
    >
      {mode === 'light' ? (
        <>
      ...
      <header className="hidden md:flex items-center justify-between px-8 py-5 mb-8 glass-panel rounded-2xl border border-base-content/5 relative z-30">
        ...
        <div className="flex items-center gap-4">
            ...
            <button 
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 hover:shadow-[0_0_15px_rgba(var(--color-primary),0.2)] transition-all font-medium text-sm"
            >
